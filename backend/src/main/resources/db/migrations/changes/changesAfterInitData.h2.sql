...
--changeset gonchar:ADD_DATE_TO_COLUMN_INTO_CALENDAR
ALTER TABLE CALENDAR ADD DATE_TO date;

--changeset gonchar:ADD_DATE_FROM_COLUMN_INTO_CALENDAR
ALTER TABLE CALENDAR ADD DATE_FROM date;

--changeset gonchar:SET_DATE_TO_COLUMN_IN_CALENDAR
UPDATE CALENDAR SET DATE_TO = (select max(DATE_TO) from EVENT);
...
--changeset eldar:ADD_PARENT_ID_COLUMN_PERIOD
ALTER TABLE PERIOD ADD PARENT_ID BIGINT;

--changeset eldar:ADD_PERIOD__PERIOD__CONSTRAINT
ALTER TABLE PERIOD ADD CONSTRAINT PERIOD__PERIOD__CONSTRAINT FOREIGN KEY (PARENT_ID) REFERENCES PERIOD (PERIOD_ID) ON UPDATE CASCADE ON DELETE CASCADE;

--changeset gonchar:LOCATION_COLUMN_IN_UNIT_IS_NULLABLE
alter table UNIT alter column LOCATION set null;
alter table UNIT alter column LOCATION drop default;