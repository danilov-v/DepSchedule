...
public interface EventTypeApi {
    ...
    @ApiOperation(value = "Удалить существующий тип события", nickname = "eventTypeTypeDelete", notes = "", authorizations = {
        @Authorization(value = "JWT")
    }, tags={ "eventType", })
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 400, message = "Bad request", response = ErrorMessageDto.class),
        @ApiResponse(code = 500, message = "Internal server error", response = ErrorMessageDto.class) })
    @RequestMapping(value = "/eventType/{type}",
        produces = { "application/json" }, 
        method = RequestMethod.DELETE)
    default ResponseEntity<Void> eventTypeTypeDelete(@ApiParam(value = "", required = true) @PathVariable("type") String type) {
        return new ResponseEntity<>(HttpStatus.OK);

    }


    @ApiOperation(value = "Редактировать существующий тип события", nickname = "eventTypeTypePut", notes = "", response = EventTypeResponseDto.class, authorizations = {
        @Authorization(value = "JWT")
    }, tags={ "eventType", })
    @ApiResponses(value = { 
    ...
    default ResponseEntity<EventTypeResponseDto> eventTypeTypePut(@ApiParam(value = "", required = true) @PathVariable("type") String type, @ApiParam(value = "") @Valid @RequestBody EventTypePutDto eventTypePutDto) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    ApiUtil.setExampleResponse(request, "application/json", "null");
